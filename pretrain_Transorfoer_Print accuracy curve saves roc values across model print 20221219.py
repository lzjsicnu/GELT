...
if train_flag:
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(epochs)):
            ...
            #lzj为了打印不同roc，这里保存小数Test_preds，添加了test_preds_float,并在最后保存
            # lzj添加~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            #保留小数点后4位
            #test_preds_float = [float('{:.1f}'.format(i)) for i in test_preds_float]
            ...
            print("lin204lzj:test_auc=", test_auc)
...
# 保存Excel到.py源文件同级目录
file.save('20221220pebg_Transformer训练情况记录.xls')


plt.figure(figsize=(6,6))
plt.title('Validation ROC')
#plt.plot(test_targets[0:9], test_preds[0:9], 'b', label = 'DKT AUC = %0.3f' %test_auc)
plt.plot(fpr, tpr, 'b', label = 'DKT AUC = %0.3f' %roc_auc)
...
plt.savefig("filename07_Transformer的AUC.png")
plt.show()
plt.close()
...
import csv
import pandas as pd
#print("lin308lzj:test_preds_float", test_preds_float)
#print("lin308lzj:len=test_preds_float", len(test_preds_float))
#print("lin308lzj:len=test_targets", len(test_targets))
dataframe = pd.DataFrame({'label':test_targets,'predict':test_preds_float})
dataframe.to_csv("test1_DKT.csv",index=False,sep=',')
