...
problems= tf.placeholder(tf.int32, shape=(tf_batch_size, 50-1))
...
with tf.variable_scope("encoder"):
    ...
    enc, lookup = embedding(x,vocab_size=input_size,num_units=200,
                                      zero_pad=False,
                                      scale=False,
                                      l2_reg=0.0,
                                      scope="enc_embed",
    ...
    seq = embedding(problems,vocab_size=skill_num,
                         num_units=200,
                         zero_pad=True,
                         scale=False,
                         l2_reg=0.0,
    ...
    for i in range(num_blocks=1):
        with tf.variable_scope("num_blocks_{}".format(i)):
            ...
            enc = feedforward(normalize(enc), num_units=[200, 200],
                                   dropout_rate=0.2, is_training=True)
            enc *= key_masks
            seq *= key_masks
            enc = normalize(enc)
...
#
enc = tf.reshape(enc, [tf.shape(x)[0] * (50 - 1), 200])
#

#

sigmoid_w = tf.get_variable("sigmoid_w", [200, skill_num])
sigmoid_b = tf.get_variable("sigmoid_b", [skill_num])
#
logits = tf.matmul(enc, sigmoid_w) + sigmoid_b
