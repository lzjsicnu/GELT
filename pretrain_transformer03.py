...
#embedding_lookup的解释，这里，是依照pro_embedding,寻找与tr_pro相同的元素,b = tf.nn.embedding_lookup(c, [1, 3]) ##查找数组中的序号为1和3的
# [bs, max_len, embed_dim],这里tf_pro是数据集中问题的tf.placeholder，
...
with tf.variable_scope('LstmNet', reuse=tf.AUTO_REUSE):
    fw_cells = []
    bw_cells = []
    cell_fw = tf.nn.rnn_cell.LSTMCell(hidden_dim/2, name='fw_LSTM')#为了得到128，这里除以2
    print("line 125:hidden_dim:", hidden_dim)
    dropcell_fw = tf.nn.rnn_cell.DropoutWrapper(cell_fw, output_keep_prob=(1-0.5))
    fw_cells.append(dropcell_fw)
    cell_bw = tf.nn.rnn_cell.LSTMCell(hidden_dim/2, name='bw_LSTM')
    dropcell_bw = tf.nn.rnn_cell.DropoutWrapper(cell_bw, output_keep_prob=(1-0.5))
    ...
    print("line 132:lstm_cell_fw:",lstm_cell_fw)
    print("line 133:lstm_cell_bw:", lstm_cell_bw)
    ...
    outputs, state = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw, lstm_cell_bw,
                                                     ...
                                                     sequence_length=tf_real_seq_len,
                                                     dtype=tf.float32)
    outputs = tf.concat(outputs, 2, name='concat1')
    print("line 143:outputs:", outputs)
    #line 143:outputs: Tensor("LstmNet/concat1:0", shape=(?, ?, 256), dtype=float32)
    #这里应该调整为128
    att_w = tf.Variable(tf.random.truncated_normal(shape=[hidden_dim , hidden_dim ]),
    ...
    qk = tf.matmul(Q, tf.transpose(K, [0, 2, 1], name='t1'), name='qk') / tf.sqrt(
        tf.constant(hidden_dim * 2, dtype=tf.float32, name='scaled_factor'),name='sqrt1')
    print("line 152:qk:", qk)
    # 现在qk的大小是[batch_size,max_len,max_len]
    # 下面开始计算mask矩阵#目前卡在这里，mask的input是什么，例子中给的输入是原始的placeholder。这里我该用哪个输出？
    #inputs = tf.placeholder(tf.int32, [None, 30], name='inputs')
    mask = tf.sign(tf_pro, name='s1')  # 大小是[batch_size,max_len]
    mask = tf.expand_dims(mask, 1, name='expand1')  # 大小是[batch_size,1,max_len]
    mask = tf.tile(mask, [1, 30, 1], name='tile1')  # 大小是[batch_size,max_len,max_len]
    ...
    #                                         initial_state=init_state,
    #                                         dtype=tf.float32)
    print("line 170:outputs:",outputs)
    # line122: outputs: Tensor("LstmNet/rnn/transpose_1:0", shape=(?, ?, 128), dtype=float32)
    #line170: outputs: (< tf.Tensor 'LstmNet/bidirectional_rnn/fw/fw/transpose_1:0' shape=(?, ?, 128) dtype = float32 >, < tf.Tensor'LstmNet/ReverseSequence:0'shape = (?, ?, 128)dtype = float32 >)
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(epochs)):
            ...
            for j in range(train_steps):
                ...
                #print("line229:feed_dict:", len(feed_dict))
