...
enc_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hidden_dim) for _ in range(2)])
dec_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hidden_dim) for _ in range(2)])
src_embedding = tf.get_variable('src_emb', [rnn_inputs, hidden_dim])
trg_embedding = tf.get_variable('trg_emb', [rnn_inputs, hidden_dim])
SHARE_EMB_AND_SOFTMAX=0
if SHARE_EMB_AND_SOFTMAX:
    softmax_weight = tf.transpose(trg_embedding)
else:
    softmax_weight = tf.get_variable('weight', [hidden_dim, len(tf_pro)])
    softmax_bias = tf.get_variable('softmax_loss', [len(tf_pro)])

src_emb = tf.nn.embedding_lookup(src_embedding, tf_pro)
...
with tf.variable_scope('encoder'):
    enc_outputs, enc_state = tf.nn.dynamic_rnn(enc_cell, src_emb, len(tf_pro), dtype=tf.float32)
with tf.variable_scope('decoder'):
    dec_outputs, _ = tf.nn.dynamic_rnn(dec_cell, trg_emb, len(tf_y), initial_state=enc_state)
