...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
enc_cell_fw = tf.nn.rnn_cell.BasicLSTMCell(hidden_dim)
enc_cell_bw = tf.nn.rnn_cell.BasicLSTMCell(hidden_dim)
dec_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.BasicLSTMCell(hidden_dim) for _ in range(2)])
    # 为源语言和目标语言分别定义词向量。
src_embedding = tf.get_variable("src_emb", [hidden_dim, inputs_embed_dim])
trg_embedding = tf.get_variable("trg_emb", [hidden_dim, inputs_embed_dim])
    # 定义softmax层的变量
SHARE_EMB_AND_SOFTMAX=0
if SHARE_EMB_AND_SOFTMAX:
    softmax_weight = tf.transpose(trg_embedding)
else:
    softmax_weight = tf.get_variable("weight", [hidden_dim, inputs_embed_dim])
    softmax_bias = tf.get_variable("softmax_bias", [inputs_embed_dim])

# 在forward函数中定义模型的前向计算图
src_emb = all_pro_embed
    #tf.nn.embedding_lookup(src_embedding,tf_pro)
#trg_emb = tf.nn.embedding_lookup(trg_embedding, tf_y)

# 在词向量上进行dropout。
#src_emb = tf.nn.dropout(src_emb, 0.2)
#trg_emb = tf.nn.dropout(trg_emb, 0.2)
with tf.variable_scope("encoder"):
    enc_outputs, enc_state = tf.nn.bidirectional_dynamic_rnn(enc_cell_fw, enc_cell_bw,
                                                             src_emb, hidden_dim,dtype=tf.float32)
    enc_outputs = tf.concat([enc_outputs[0], enc_outputs[1]], -1)

with tf.variable_scope("decoder"):
    attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(hidden_dim,
                                                               enc_outputs,
                                                               memory_sequence_length=hidden_dim)
    attention_cell = tf.contrib.seq2seq.AttentionWrapper(dec_cell, attention_mechanism,
                                                             attention_layer_size=hidden_dim)
    dec_outputs, _ = tf.nn.dynamic_rnn(attention_cell, rnn_inputs, hidden_dim, dtype=tf.float32)

    # 计算解码器每一步的log perplexity。这一步与语言模型代码相同。
    output = tf.reshape(dec_outputs, [-1, hidden_dim])
